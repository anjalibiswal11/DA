...
VALUES 
('sidhart', 'gupta', 3, 70000, '2024-01-01', 'siddharth.gupta@example.com'), 
...
INSERT INTO Employees (FirstName, LastName, DepartmentID, Salary, DateOfJoining, Email)
VALUES 
('shen', 'rao', 2, 60000, '2024-02-15', 'sneha.rao@example.com');
...
select departmentname, annualbudget from departments
...
select departmentname, avg(annualbudget) from departments
group by departmentname;

-- Situation -15: For funding efficiency, find the maximum and minimum budgets among departments in each city. 
select departmentname, max(annualbudget) , min(annualbudget) from departments
group by departmentname;
...
select 
    departmentid,
    concat(departmentname, ', ', location) as completeaddress
from departments;
...
select 
    ...
    firstname,
    lastname,
    concat(mobileno, ifnull(concat(' ext-', extension), '')) as companycontact
from employees;
...
select 
    employeeid,
    concat('dear ', firstname, ' (', jobtitle, ' - ', departmentname, ')') as notificationtext
from employees;
...
select 
    employeeid,
    firstName,
    concat(year(curdate()), '-', firstName) as annualreport
from employees;
...
delete from employees
where year(dateofjoining) < 2020 and projectstatus = 'deactive';
...
delete from employees
where region = 'north' and year(employmentenddate) < 2023;
...
-- insert a new employee without specifying status
insert into employees (firstname, lastname, departmentid, salary, dateofjoining, email)
values ('test', 'user', 1, 50000, '2025-01-01', 'test.user@example.com');

-- check the new employee record
...
select * from employees_partitioned partition (p0); 
select * from employees_partitioned partition (p1); 
